...
SOH_k = 0.94 # estimated SOH at k, it would be estimate based on deep learning model..
...
for battery in train_battery_list:
    ...
    OCV_k2, SOC_k2 = SOCcurve(SOC_OCV_1, Capacity_1, T_1, (SOH_k - d_SOH_k) * BatterySpec.spec_full_capacitor)
...
Ccs = 90*(1-0.018) #90
Ccb = 12240*(1-0.018)  #  12240  # Ccb = 3370.7558     1.1 * 3600  ,  3.4 * 3600
Rt = 0.06*(1+0.474)  #0.06

d_Ccs = Ccs*0.0226  #0.38
d_Ccb = Ccb*0.0226 #0.38

# d_Rt = 10.86
d_Rt = Rt*0.5488 #0.01086
...
d_kp = d_kp = np.abs(kp - soc_k_mapping2(init_soc))  #0.007
...
def AdaptiveCovarianceProjectionFilter(Ad, Bd, Cd, Dd, Ri, u, d, P, Q, R, xhat, ytrue, k, uc1, uc2, uc3):
    ...
    xaugmentedfuse[k, 2, 0] = ytrue[k] - Ri*u[k] - d- uncertainty3[k]
    ...
    uncertainty1[0] = (1 / (kp*Ccb)) * (
                (u[0] / 1000 * 0.0005) - (u[0] / 1000  * soc_k_mapping(xhat[0, 0]) * d_kp * (u[0] / 1000 / Ccb) * d_Ccb))
    uncertainty2[0] = (1 / (Rt * Ccb)) * (
                (ysimulation[0] * Ccs / Ccb / Rt * d_Rt) + (ysimulation[0] * Ccs / Ccb / Ccs * d_Ccs)) + (
                                  1 / Ccs) * ((u[0] / 1000  * 0.0005) - (u[0] / 1000  * d_Ccs / Ccs))

    uncertainty3[0] = ysimulation[0] *0.0005
...
for k in range(1, n_iter):
    ...
    if k> 5000 and k< 10000: # 10000:  #7500
    ...
    kp = soc_k_mapping(xsimulation[k, 0])
    d_kp = d_kp = np.abs(kp - soc_k_mapping2(xsimulation[k, 0]))
    ...
    uncertainty1[k] = (1 / (soc_k_mapping(xhat[k - 1, 0]) * Ccb)) * (
                (u[k] * 0.0005) - (u[k] * soc_k_mapping(xhat[k - 1, 0]) * d_kp * (u[k] / Ccb) * d_Ccb))
    uncertainty2[k] = (1 / (Rt * Ccb)) * (
                (ysimulation_noisy[k] * Ccs / Ccb / Rt * d_Rt) + (ysimulation_noisy[k] * Ccs / Ccb / Ccs * d_Ccs)) + (
                                  1 / Ccs) * ((u[k] * 0.0005) - (u[k] * d_Ccs / Ccs))
    uncertainty3[k] = ysimulation_noisy[k] * 0.0005

    Q[k] = np.array([[uncertainty1[k] * uncertainty1[k], uncertainty1[k] * uncertainty2[k]],
                     [uncertainty1[k] * uncertainty2[k], uncertainty2[k] * uncertainty2[k]]])

    AdaptiveCovarianceProjectionFilter(Ad, Bd, Cd, Dd, init_Ri, u, d, P, Q, R, xhat, ysimulation_noisy, k, uncertainty1, uncertainty2, uncertainty3)
...
plt.plot(x, k_sequence_record, label=r'Simulation ${k^p}(t)$')
plt.plot(x, d_sequence_record, label=r'Simulation ${d^p}(t)$')
plt.plot(x, k_sequence_model_record, label=r'Model ${k^p}(t)$')
plt.plot(x, d_sequence_model_record, label=r'Model ${d^p}(t)$')
...
plt.figure()
prepare_dist = np.load(path+'/../temp_save_data_simul3.npy', allow_pickle=True)
# plt.plot(x, prepare_dist, label='distance (Constant Uncertainty)', color='orange')
