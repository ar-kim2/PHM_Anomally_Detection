...
'''
Maryland dataset을 이용하여 시뮬레이터로 이상치를 추가하고
이상징후를 검출한 결과
'''
...
for battery in train_battery_list:
    ...
    plt.figure()
    plt.plot(np.flip(SOC_base, 0).reshape(-1), np.flip(OCV_base[1:-1], 0).reshape(-1), label='base battery')
    plt.xlabel('SOC')
    plt.ylabel('OCV')
...
n_iter = int(init_data_size*50)             # 30초 sampling한 data로 simulation하면서 T =1 로 설정했기 때문에, k,d구하면서 잘라낸 부분 고려하면 *50정도는 해주어야 배터리 노화가 simuation된다.
