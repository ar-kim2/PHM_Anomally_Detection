...
SOH_k = 0.964  #   97   # estimated SOH at k, it would be estimate based on deep learning model..
...
for battery in train_battery_list:
    ...
    OCV_k2, SOC_k2 = SOCcurve(SOC_OCV_1, Capacity_1, T_1, (SOH_k-d_SOH_k) * BatterySpec.spec_full_capacitor)        # charging direction

    K_base, D_base = get_KnD(OCV_base, SOC_base)  # discharging direction
    K_k, D_k = get_KnD(OCV_k, SOC_k) # discharging direction
    K_k2, D_k2 = get_KnD(OCV_k2, SOC_k2) # discharging direction
    ...
    for i in range(10):
        K_k = np.delete(K_k, 0)
        K_k2 = np.delete(K_k2, 0)
        D_k = np.delete(D_k, 0)
        D_k2 = np.delete(D_k2, 0)
        K_k = np.delete(K_k, -1)
        K_k2 = np.delete(K_k2, -1)
        D_k = np.delete(D_k, -1)
        D_k2 = np.delete(D_k2, -1)
    ...
    soc_k_mapping2 = InterpolatedUnivariateSpline(SOC_k2, np.flip(K_k2, 0), k=2)
...
Ccs = 90*(1-0.0091)   #  90
Ccb = 12240*(1-0.0091)  #12240
Rt = 0.06*(1+0.221) # 0.06

d_Ccs = Ccs*0.0057 #0.38
d_Ccb = Ccb*0.0057  #0.38

# d_Rt = 10.86
d_Rt = Rt*0.3396 #0.01086
...
def AdaptiveCovarianceProjectionFilter(Ad, Bd, Cd, Dd, Ri, u, d, P, Q, R, xhat, ytrue, k, uc1, uc2, uc3):
    '''
    # ZHAO comments:
    ...
    """ADD Uncertainity"""
    xhatminus[k] = np.dot(Ad, xhat[k-1]) + Bd*u[k-1] + U1  # EKF Step 1a: State estimate time update
    ...
    # print("CHECK xhatminus[k] ", xhatminus[k])
    ...
    """ADD Uncertainity"""
    xaugmentedfuse[k, 2, 0] = ytrue - Ri*u[k] - d - uncertainty3[k]
    ...
    if dist[k] > 3 : #3.8:   # 7
        ...
        Paugmented2[2, 2] = 999999999  # Paugmented2[2, 2] *100 #99999999999

        Q[k] = np.array([[0.0000008, 0], [0, 0.0000002]])
        ...
        dist[k] = np.asmatrix(xaugmentedhat_w - xtide_w).T.dot(np.asmatrix(xaugmentedhat_w - xtide_w))
        dist[k] = (xaugmentedhat[k] - xtide[k]).T.dot(_Paugmented.I).dot(xaugmentedhat[k] - xtide[k]) + 3
...
FileNameList = ['065A_10t1ohm.xls', '065A_5m5m1_5ohm.xls']
...
for data in range(np.size(FileNameList)):
    ...
    d_kp = np.abs(kp - soc_k_mapping2(init_soc))
    ...
    uncertainty1 = np.zeros((n_iter,))
    uncertainty2 = np.zeros((n_iter,))
    uncertainty3 = np.zeros((n_iter,))
    ...
    uncertainty1[0] = (1 / (kp*Ccb)) * (
                (ymeasure_cur[0][0] / 1000 * 0.0005) - (ymeasure_cur[0][0] / 1000  * soc_k_mapping(xhat[0, 0]) * d_kp * (ymeasure_cur[0][0] / 1000 / Ccb) * d_Ccb))
    uncertainty2[0] = (1 / (Rt * Ccb)) * (
                (ymeasure[0][0] * Ccs / Ccb / Rt * d_Rt) + (ymeasure[0][0] * Ccs / Ccb / Ccs * d_Ccs)) + (
                                  1 / Ccs) * ((ymeasure_cur[0][0] / 1000  * 0.0005) - (ymeasure_cur[0][0] / 1000  * d_Ccs / Ccs))

    uncertainty3[0] = ymeasure[0][0] *0.0005
    ...
    for k in range(1, n_iter):
        ...
        kp = soc_k_mapping(xhat[k-1, 0])
        d_kp = d_kp = np.abs(kp - soc_k_mapping2(xhat[k-1, 0]))

        uncertainty1[k] =  (1/(soc_k_mapping(xhat[k-1, 0])*Ccb))*((u[k]*0.0005)-(u[k]*soc_k_mapping(xhat[k-1, 0])*d_kp*(u[k]/Ccb)*d_Ccb))
        uncertainty2[k] =  (1/(Rt*Ccb))*((ymeasure[0][y_idx]*Ccs/Ccb/Rt*d_Rt)+(ymeasure[0][y_idx]*Ccs/Ccb/Ccs*d_Ccs))+(1/Ccs)*((u[k]*0.0005)-(u[k]*d_Ccs/Ccs))
        uncertainty3[k] =  ymeasure[0][y_idx]*0.0005
        #
        # print("CHECK u1 ",  uncertainty1[k])
        # print("CHECK u2_1 ",  (1/(Rt*Ccb))*((ymeasure[0][y_idx]*Ccs/Ccb/Rt*d_Rt)+(ymeasure[0][y_idx]*Ccs/Ccb/Ccs*d_Ccs)))
        # print("CHECK u2_2 ",  (1/Ccs)*((u[k]*0.05)-(u[k]*d_Ccs/Ccs)))
        #
        # print("CHECK u2 ",  uncertainty2[k])

        """ADD Uncertainity"""
        Q[k] = np.array([[uncertainty1[k]*uncertainty1[k], uncertainty1[k]*uncertainty2[k]], [uncertainty1[k]*uncertainty2[k], uncertainty2[k]*uncertainty2[k]]])


        # Q[k] = np.array([[uncertainty1[k]*uncertainty1[k], 0], [0, uncertainty2[k]*uncertainty2[k]]])

        # print("CHECK Q ", Q[k])



        AdaptiveCovarianceProjectionFilter(Ad, Bd, Cd, Dd, init_Ri, u, d, P, Q, R, xhat, ymeasure[0][y_idx], k, uncertainty1, uncertainty2, uncertainty3)


        #if k >9 and k < 30:


    x = np.array(range(1, n_iter - 2))
    ...
    if data == 1:
        # np.save(path+'/../temp_save_data1.npy', dist[30:])


        prepare_dist = np.load(path+'/../temp_save_data1.npy', allow_pickle=True)

        # plt.plot(x[:-27], prepare_dist, label='distance  (Constant Uncertainty)', color='orange')

        plt.plot(x[:-27], dist[30:], label='distance ', color='blue')

        plt.plot(x, line, color='r')
    else:
        # np.save(path + '/../temp_save_data2.npy', dist[3:8503])


        prepare_dist = np.load(path + '/../temp_save_data2.npy', allow_pickle=True)
        # plt.plot(x[:8500], prepare_dist, label='distance (Constant Uncertainty)', color='orange')

        plt.plot(x[:8500], dist[3:8503], label='distance ', color='blue')

        plt.plot(x[:8500], line[:8500], color='r')
        # plt.plot(dist[3:], label='distance ', color='blue')
