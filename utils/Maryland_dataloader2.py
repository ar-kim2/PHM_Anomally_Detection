...
if __name__ == "__main__":
    ...
    for battery in battery_list:
        ...
        if save:
            ...
            for j in range(len(DC_list[0])):
                for i in range(len(DC_list)):
                    All_DC_list.append(DC_list[i][j])
                    All_DC_list_current.append(DC_list_current[i][j])
                    All_DC_list_time.append(DC_list_time_all[i][j])

            print("check 0:", len(capacity_list1))
            print("check 1: ", len(All_DC_list))
            print("check 2: ", len(All_DC_list_current))
            print("check 3: ", len(All_DC_list_time))

            np.save(battery_path + '/total/capacity', capacity_list1)
            np.save(battery_path + '/total/discharge_data', All_DC_list)
            np.save(battery_path + '/total/discharge_current', All_DC_list_current)
            np.save(battery_path + '/total/discharge_time_all', All_DC_list_time)
