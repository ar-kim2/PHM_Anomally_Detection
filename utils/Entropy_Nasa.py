...
def Entropy2(density, len_value, current_value, beta):
    ...
    ent = -1 * beta * ent1 * (1.041 / np.exp(0.0204 * current)) / (current*time_compenate)
...
def Entropy2_2(density, len_value, current_value, beta, time_rate):
    ...
    ent = -1 * beta * ent1  / (integ_CB)  # (current * len_value)  #

    #ent = -1 * beta * ent1 * (1.032 / np.exp(0.026 * current)) / (integ_CB)  #(current * len_value)  #

    #ent = -1 * beta * ent1 * (35.86 * np.exp((-0.247) * current) - 34.951 * np.exp((-0.258) * current)) / (integ_CB)  # (current * len_value)  #
...
if __name__ == "__main__":
    ...
    #battery_list = ['B0046', 'B0047','B0048']
    #battery_list = ['B0033']
    ...
    for battery in battery_list:
        ...
        for j in range(len(DC_Entropy)):
            abs_current = np.abs(DC_list_current[j])
            current = 1.3 #np.average(abs_current)
            integ_CB = integrate.cumtrapz(abs_current, All_DC_time[j])[-1]
            ...
            DC_Entropy[j] = DC_Entropy[j] * (alpha / (integ_CB*(0.944 * np.exp(0.045 * current))))
            #DC_Entropy[j] = DC_Entropy[j] * (alpha / (All_DC_time[j][-1] * current))
            #DC_Entropy[j] = DC_Entropy[j] * (alpha / (All_DC_time[j][-1]*current*(0.944 * np.exp(0.045 * current))))
